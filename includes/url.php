...
function ibup_process_background_fragment($fragment, $hosts) {
  ...
  $fragment = preg_replace("/(\sstyle=['\"][^>]*?)(background-image:.*?url\((\s*.*?\s*)\));?(.*?['\"])/xis", '$1$4 imageus-bg-src=$3', $fragment);
  $fragment = preg_replace("/(\sstyle=['\"][^>]*?background:.*?)(url\((\s*.*?\s*)\));?(.*?['\"])/xis", '$1$4 imageus-bg-src=$3', $fragment);
...
function ibup_process_image_fragment($img, $hosts) {
  ...
  $img = preg_replace('/<img(.*?)src=(["\']?).*?[\'">\s\r\n]/is', '<img$1src="' . $transparent_src . '" imageus imageus-src="' . $src . '"', $img);
